...
def draw_card(card):
    """Displays the card on the UI

    Parameters:
    card - the card to be displayed (Should be of type Game_Card)
    """
...
def draw_cards_in_hand():
    """Displays the cards that are being moved (dragged) by the player, if there are any"""
...
def draw_card_slots():
    """Displays the card slots on the UI"""
...
def draw_board():
    """Displays the game board with all the assets on it"""
    draw_deck_section()
    draw_card_slots()
...
def draw_window():
    """Displays all the required components for a frame to represent the Solitaire Game UI"""
